 ...
 * @brief C610电调驱动程序（适用于M2006动力电机系统）
 * @version
 * @date 2024-04-05
 * 
 ...
 * @param ec 电调设备对象
 */
__STATIC_FORCEINLINE void rt_c610_recv_parser(rt_c610_t ec)
{
  if (ec->rx_buff_ptr == RT_NULL)
    return;
  
  uint8_t id = (*(uint16_t*)(ec->rx_buff_ptr + 8) - C610_HEADER_RET_BASE);
  ...
  if (id > 4 && ec->header != C610_HEADER_CTRL_GROUP_2 || id <= 4 && ec->header != C610_HEADER_CTRL_GROUP_1)
    return;
 ...
 * @param ec 电调设备对象
 */
__STATIC_FORCEINLINE void rt_c610_send_packer(rt_c610_t ec)
{
 ...
 * @param ec 电调设备对象
 */
static void rt_c610_refresh(rt_c610_t ec)
{
 ...
 */
static rt_err_t rt_c610_instance_open(rt_device_t dev, rt_uint16_t oflag)
{
  RT_ASSERT(dev != RT_NULL);
  rt_c610_t ec = (rt_c610_t)dev;
 ...
 * @return rt_err_t 操作结果，目前没做异常返回
 */
static rt_err_t rt_c610_instance_close(rt_device_t dev)
{
  RT_ASSERT(dev != RT_NULL);
  rt_c610_t ec = (rt_c610_t)dev;
...
/**
 * @brief 读取电调状态到buffer中，格式为struct rt_c610_ctrl
 * 
 * @param dev 设备对象
 ...
 */
static rt_size_t rt_c610_instance_read(rt_device_t dev, rt_off_t pos, void *buffer, rt_size_t size)
{
  RT_ASSERT(dev != RT_NULL);
  ...
  rt_memcpy(buffer, &ec->ctrl[start], sizeof(end - start + 1) * sizeof(struct rt_c610_ctrl));
 ...
 */
static rt_size_t rt_c610_instance_write(rt_device_t dev, rt_off_t pos, const void *buffer, rt_size_t size)
{
  RT_ASSERT(dev != RT_NULL);
  RT_ASSERT(buffer != RT_NULL);
...
static const struct rt_device_ops rt_c610_ops = {
  .open   = rt_c610_instance_open,
  .close  = rt_c610_instance_close,
  .read   = rt_c610_instance_read,
  .write  = rt_c610_instance_write
};
#endif
 ...
 * @brief 创建C610电调设备对象
 * 
 * @return rt_c610_t 设备对象指针
 */
rt_c610_t rt_c610_create(void)
{
  rt_c610_t ec = (rt_c610_t)rt_device_create(RT_Device_Class_Miscellaneous, sizeof(struct rt_c610) - sizeof(struct rt_device));
  if (ec != RT_NULL)
...
rt_err_t rt_c610_register(rt_c610_t ec, const char *name)
{
  ...
  RT_ASSERT(ec->header == C610_HEADER_CTRL_GROUP_1 || ec->header == C610_HEADER_CTRL_GROUP_2);
  ...
  dev->ops      = &rt_c610_ops;
  #else
  dev->open     = rt_c610_instance_open;
  dev->close    = rt_c610_instance_close;
  dev->read     = rt_c610_instance_read;
  dev->write    = rt_c610_instance_write;
  #endif
  ...
  rt_timer_init(&ec->refresh_timer, timer_name, (void(*)(void*))rt_c610_refresh, ec, 250, RT_TIMER_FLAG_PERIODIC);
  return RT_EOK;
}
...
/**
 * @brief 从CAN更新数据到C610电调设备对象，建议添加到CAN接收中断中运行
 * 
 * @param ec 设备对象指针
 * @param hcan CAN接口对象
 * @param header CAN帧头
 * @param data CAN数据
 */
__INLINE void rt_c610_recv(rt_c610_t ec, CAN_HandleTypeDef *hcan, CAN_RxHeaderTypeDef *header, uint8_t *data)
{
  RT_ASSERT(ec != RT_NULL);
  ...
  if (hcan->Instance != ec->interface->Instance ||\
      header->IDE != CAN_ID_STD ||\
      header->RTR != CAN_RTR_DATA ||\
      (header->StdId & 0xFFF8) != C610_HEADER_RET_BASE ||\
      header->DLC != 8)
    return;
 ...
 * @param hcan CAN接口对象
 */
__INLINE void rt_c610_send(rt_c610_t ec, CAN_HandleTypeDef *hcan)
{
  RT_ASSERT(ec != RT_NULL);
  RT_ASSERT(hcan != RT_NULL);
...
/**
 * @brief C610电调状态显示命令（接入到RT-Thread shell）
 * 
 * @param argc 命令参数个数
 * @param argv 命令参数字符串
 */
static void c610_show_stat(int argc, char *argv[])
{
  rt_device_t dev = RT_NULL;
  rt_c610_t ec = RT_NULL;
  ...
  while (HAL_UART_GetState(&huart6) != HAL_UART_STATE_READY && HAL_UART_GetState(&huart6) != HAL_UART_STATE_BUSY_RX)
    rt_thread_mdelay(1);
  rt_strncpy(output_buffer, "id\t|angle/deg\t|speed/rpm\t|current/A\t|ping/tick\n\r", sizeof(output_buffer));
  HAL_UART_Transmit_DMA(&huart6, (const uint8_t*)output_buffer, rt_strnlen(output_buffer, sizeof(output_buffer)));
  for (int i = 0; i < 4; i++)
  {
    int id;
    if (ec->header == C610_HEADER_CTRL_GROUP_1)
      id = i + 1;
    else
      id = i + 5;
    ...
    snprintf(output_buffer, sizeof(output_buffer), "%d\t|%f\t|%d\t\t|%f\t|%u\n\r",\
    id,\
    ec->ctrl[i].angle.u16 * 360.0f / 8192.0f,\
    (int)ec->ctrl[i].speed.i16,\
    ec->ctrl[i].torque.i16 * 10.0f / 10000.0f,\
    ...
    //rt_kprintf("%d\t|%f\t\t|%f\t\t|%f\t\t|%d\n", id, ctrl.angle.u16 * 360.0f / 8192.0f, ctrl.speed.i16 * 20.0f / 16384.0f, ctrl.current_read.i16, (int)ctrl.temp.i8);
  }
}
MSH_CMD_EXPORT(c610_show_stat, display c610 ec status by giving device name.);

/**
 * @brief 设置C610电调输出电流（接入到RT-Thread shell）
 * 
 * @param argc 命令参数个数
 * @param argv 命令参数字符串
 */
static void c610_set_amp(int argc, char *argv[])
{
  rt_device_t dev = RT_NULL;
  rt_c610_t ec = RT_NULL;
  extern UART_HandleTypeDef huart6;
  ...
  int16_t i16_current = current * 10000.0f / 10.0f;
  rt_device_write(dev, id, &i16_current, 1);
}
MSH_CMD_EXPORT(c610_set_amp, set c610 ec output current by giving device name and id.);